...
def update_scan_progress(db: Session, warehouse_id: str,
                         ...
                         current_pos: tuple[int, int, int] | None = None) -> None:
...
def persist_scan_results(db: Session, warehouse_id: str,
    ...
    # 1. Rebuild aggregated material_items (delete → insert)
    ...
    # 2. Rebuild container_items (one row per container: primary item + total)
