"""Litematica Parser — parses .litematic files using nbtlib.

Extends vmtools-backend's litematic_parser.py with:
  - Region/layer slicing
  - Palette → BlockState mapping
  - Alignment with Java LitematicaAdapter method signatures
  - TTLCache for repeated reads

.litematic format:
  gzip → NBT → {Metadata: {...}, Regions: {name: {Position: {...}, Size: {...},
    BlockStatePalette: [...], BlockStates: [long...]}}}
"""
...
logger = logging.getLogger("vmtools.litematica")

# Block → Item mapping (common blocks, extensible)
BLOCK_TO_ITEM = {
...
def _block_state_to_item_id(block_state: str) -> str:
    """Convert a block state name to an item ID.

    E.g. "minecraft:oak_stairs[facing=north,half=bottom,shape=straight]"
    → "minecraft:oak_stairs"
    """
    base = block_state.split("[")[0] if "[" in block_state else block_state
    return BLOCK_TO_ITEM.get(base, base)
...
class ParsedProjection:
    ...
    def __init__(self):
        ...
        self.regions: dict[str, dict] = {}  # name → {size, position, palette, block_states}
        self.materials: dict[str, int] = {}  # item_id → count
        self.layers: dict[int, list[tuple[int, int, int, str]]] = {}  # y_offset → [(x,y,z,block_state)]


class LitematicaParser:
    """Parses .litematic files with TTLCache."""

    _cache: TTLCache = TTLCache(maxsize=16, ttl=3600)

    @classmethod
    async def parse_file(cls, file_path: str) -> ParsedProjection:
        """Parse a .litematic file and return structured data."""
        # Check cache
        file_hash = _file_sha256(file_path)
        if file_hash in cls._cache:
            logger.debug("Litematica cache hit: %s", file_path)
            return cls._cache[file_hash]

        logger.info("Parsing litematic: %s", file_path)
        ...
        parsed.author = str(metadata.get("Author", ""))
        parsed.description = str(metadata.get("Description", ""))
        parsed.total_volume = int(metadata.get("TotalVolume", 0))
        parsed.total_blocks = int(metadata.get("TotalBlocks", 0))

        # Regions
        regions = nbt_data.get("Regions", {})
        materials: dict[str, int] = {}
        layers: dict[int, list[tuple[int, int, int, str]]] = {}
        ...
        for region_name, region_data in regions.items():
            ...
            if sx < 0: sx, px = -sx, px + sx
            if sy < 0: sy, py = -sy, py + sy
            if sz < 0: sz, pz = -sz, pz + sz

            # Palette (preserve Properties for directional blocks)
            ...
            for entry in palette:
                ...
                if props:
                    ...
                    palette_names.append(f"{name}[{prop_str}]")
                else:
                    palette_names.append(name)

            # Block states (bit-packed)
            block_states_raw = region_data.get("BlockStates", [])
            palette_bits = max(2, math.ceil(math.log2(max(len(palette_names), 1))))

            # Decode block states
            block_indices = _decode_block_states(block_states_raw, palette_bits, sx * sy * sz)
            ...
            parsed.regions[str(region_name)] = region_info

            # Map blocks to materials and layers
            for idx, palette_idx in enumerate(block_indices):
                ...
                # Material count
                item_id = _block_state_to_item_id(block_name)
                materials[item_id] = materials.get(item_id, 0) + 1

                # Layer grouping (by y offset)
                if y not in layers:
                    layers[y] = []
                layers[y].append((world_x, world_y, world_z, block_name))

        parsed.materials = materials
        parsed.layers = layers

        # Cache
        cls._cache[file_hash] = parsed
        logger.info("Parsed litematic: %d blocks, %d materials, %d layers",
                     parsed.total_blocks, len(materials), len(layers))
        return parsed

    @classmethod
    async def get_projection_info(cls, file_path: str,
                                    origin_x: int = 0, origin_y: int = 0,
                                    origin_z: int = 0) -> ProjectionInfo:
        """Get projection metadata (aligns with Java LitematicaAdapter.getProjectionInfo)."""
        parsed = await cls.parse_file(file_path)
        max_x = max((r["size"]["x"] for r in parsed.regions.values()), default=0)
        max_y = max((r["size"]["y"] for r in parsed.regions.values()), default=0)
        max_z = max((r["size"]["z"] for r in parsed.regions.values()), default=0)
    ...
    @classmethod
    async def get_material_requirements(cls, file_path: str) -> list[ProjectionMaterialRequirement]:
        """Get material requirements (aligns with Java getMaterialRequirements)."""
        parsed = await cls.parse_file(file_path)
        return [
            ...
            for item_id, count in parsed.materials.items()
        ]

    @classmethod
    async def get_layer_blocks(cls, file_path: str, layer_index: int,
                                layer_height: int = 6,
                                origin_x: int = 0, origin_y: int = 0,
                                origin_z: int = 0) -> list[tuple[int, int, int, str]]:
        """Get blocks for a specific layer (world coordinates).

        Returns [(world_x, world_y, world_z, block_state), ...]
        """
        parsed = await cls.parse_file(file_path)
        result = []
        y_start = layer_index * layer_height
        y_end = y_start + layer_height

        for y_offset, blocks in parsed.layers.items():
            if y_start <= y_offset < y_end:
                for (wx, wy, wz, block_state) in blocks:
                    result.append((
                        origin_x + (wx - list(parsed.regions.values())[0]["position"]["x"]),
                        origin_y + (wy - list(parsed.regions.values())[0]["position"]["y"]),
                        origin_z + (wz - list(parsed.regions.values())[0]["position"]["z"]),
                        block_state,
                    ))
        return result


def _decode_block_states(raw_data, bits_per_entry: int, expected_count: int) -> list[int]:
    """Decode bit-packed block states array.

    Minecraft stores block states as an array of longs, each containing
    multiple block indices packed at bits_per_entry bits each.
    """
    if not raw_data:
        return [0] * expected_count

    indices = []
    masks_per_long = 64 // bits_per_entry if bits_per_entry > 0 else 64
    mask = (1 << bits_per_entry) - 1

    for long_val in raw_data:
        if long_val < 0:
            long_val += 2**64
        for i in range(masks_per_long):
            if len(indices) >= expected_count:
                break
            indices.append((long_val >> (i * bits_per_entry)) & mask)

    return indices[:expected_count]
